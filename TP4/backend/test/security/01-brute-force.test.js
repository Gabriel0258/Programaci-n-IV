...
// 🔥 Store del brute force (hay que limpiarlo antes de cada test)
const { clearStore } = require('../../src/utils/bruteForce');

describe('Seguridad: Brute Force', () => {
  let app;

  beforeEach(() => {
    // 🔥 Limpia completamente los intentos antes de cada test
    clearStore();
  ...
  test(
    '❌ DEBE FALLAR: El endpoint de login debe tener protección contra fuerza bruta',
    async () => {
      const loginAttempts = [];
